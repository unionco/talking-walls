	...
	public function __construct() {
		...
		add_filter(
			...
			function( $plugins ) {
				...
				unset( $plugins['sucuri-scanner/sucuri.php'] );
		...
		add_filter( 'wp_direct_php_update_url', function ( $direct_update_url ) {
			...
			// MT cannot currently upgrade PHP.
			if ( Plugin::is_mt() ) {
			...
			return $url ? untrailingslashit( $url ) . '&showPhpModal' : $direct_update_url;

		} );

		/**
		 * Do some overrides if we are using the bundled Gutenberg.
		 */
		if ( in_array( 'gutenberg', Plugin::bundled_plugins_loaded(), true ) ) {

			// Turn off experiments.
			add_filter( 'option_gutenberg-experiments', '__return_empty_array' );

			// Remove Gutenberg from the admin menu.
			...
			// Prevent direct access to any of the Gutenberg admin pages.
			add_action( 'admin_init', function () {

				global $pagenow;

				if ( 'admin.php' === $pagenow && in_array( filter_input( INPUT_GET, 'page' ), [ 'gutenberg', 'gutenberg-experiments' ], true ) ) {
